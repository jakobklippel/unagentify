...
import { BashTool, EditTool, GlobTool, GrepTool, ReadTool, WriteTool } from '@loopstack/remote-client';

const ImplementationInputSchema = z.object({
  plan: z.string(),
});

type ImplementationInput = z.infer<typeof ImplementationInputSchema>;

@Workflow({
  uiConfig: __dirname + '/implementation-agent.ui.yaml',
  schema: ImplementationInputSchema,
})
export class DemoImplementationAgentWorkflow extends BaseWorkflow {
  ...
  @Initial({ to: 'ready' })
  async setup(args: ImplementationInput) {
